...
import { openOrder, closeOrder } from "../../redux/orderSlice";

const Order = () => {

    const isOrder = false
    //! Мы также используем хук useSelector для получения значения isOpen из хранилища Redux.
    const isOpen = useSelector((state) => {
        return state.order.isOpen
    })
    //! В этом примере мы используем хук useDispatch для получения функции dispatch, которая позволяет отправлять действия в хранилище Redux.
    const dispatch = useDispatch();

    // ! В функциях handleCloseOrder мы отправляем действия openOrder и closeOrder соответственно, чтобы обновить состояние isOpen в хранилище Redux.

    const handleCloseOrder = () => {
        dispatch(closeOrder());
    };

    // ! не отображать если модальное окно если IsOpen не true
    if (isOpen == false) return null

    // ! Если есть заказ открыть одно готового заказа
    ...
    return (
        <div className="order">
            <div className="order__wrapper">
                ...
                <button className="order__close" type="button" onClick={handleCloseOrder}>×</button>
            </div>
        </div>
    )
